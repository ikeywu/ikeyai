    ...
    grid.innerHTML = tools.map(tool => {
        ...
        <div class="tool-card" data-id="${tool.id}" onclick="showToolDetail(${tool.id})">
            <div class="tool-header">
                ...
                <div class="tool-info">
                    <h3>${tool.name} ${tool.hot ? '<span class="hot-badge">🔥 热门</span>' : ''}</h3>
...
function renderFavorites() {
    ...
    if (favoriteTools.length === 0) {
        container.innerHTML = '<p class="empty-favorites">还没有收藏工具，点击工具卡片上的 ❤️ 收藏</p>';
    } else {
...
function renderStats() {
    ...
    statsContainer.innerHTML = `
        <div class="stat-item">
            ...
            <span class="stat-label">收录工具</span>
        </div>
        <div class="stat-item">
            <span class="stat-number">${hotTools}</span>
            <span class="stat-label">热门工具</span>
        </div>
        <div class="stat-item">
...
function debounceSearch() {
    ...
    searchTimeout = setTimeout(searchTools, 300);
}
    ...
    detail.innerHTML = `
        <div class="tool-detail-header">
            ...
            <div class="tool-detail-info">
                <h2>${tool.name} ${tool.hot ? '<span class="hot-badge">🔥 热门</span>' : ''}</h2>
        ...
        <div class="tool-detail-footer">
            ...
            <button class="favorite-btn-large ${favorites.includes(tool.id) ? 'active' : ''}" onclick="toggleFavorite(${tool.id})">
...
function toggleFavorite(toolId) {
    ...
    if (currentCategory === 'all') {
        ...
        renderTools(aiTools.filter(t => t.category === currentCategory));
    }
    renderFavorites();
}
